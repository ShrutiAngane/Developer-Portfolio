      ...
      <div className='flex flex-col justify-evenly items-center md:grid md:grid-cols-2 gap-[30px] md:gap-[60px] mt-5'>
        {projects.map((element)=>{
          return <div key={element.id} className={`flex flex-col items-center h-[300px] w-[300px] mt-5 rounded-lg hover:cursor-pointer hover:scale-105 duration-500 ${theme?'customshadow-dark':'customshadow-light border-2 border-teal-500'}`}>
            ...
            <h4 className={`${theme?'text-[#FFFFFF]':'text-[#706C6C]'} text-[16px] font-medium m-[22px] font-bree`}>{element.title}</h4>
            <div className='flex justify-evenly items-center w-full border-t-2 h-full'>
              <div className='w-full flex justify-center border-r-2 h-full items-center'><a href={element.demo} target='_blank' rel="noreferrer" className={`${theme?'text-[#FFFFFF]':'text-[#706C6C]'} text-[16px] font-normal font-bree`}>Demo</a></div>
              <div className='w-full flex justify-center h-full items-center'><a href={element.code} target='_blank' rel="noreferrer" className={`${theme?'text-[#FFFFFF]':'text-[#706C6C]'} text-[16px] font-normal font-bree`}>Code</a></div>
            </div>
          </div>
        })}
