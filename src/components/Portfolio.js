    ...
    <section className={`${props.mode?'bg-gray-900':''} flex flex-col justify-evenly items-center max-w-full`} name='portfolio'>
      ...
      <div className='flex flex-col justify-evenly items-center md:grid md:grid-cols-2 gap-[30px] md:gap-[60px] mt-5'>
        {projects.map((element)=>{
          return <div key={element.id} className={`flex flex-col items-center h-[320px] w-[300px] mt-5 rounded-lg hover:cursor-pointer hover:scale-105 duration-500 shadow-lg ${props.mode?'shadow-teal-500':'shadow-[#ACAAB0] border-2 border-teal-500'}`}>
